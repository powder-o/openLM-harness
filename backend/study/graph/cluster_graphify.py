"""Vendored, unmodified (apart from this header) from the graphify project:
  source: graphify/cluster.py
  project: graphify-8 (powder-o/graphify, "graphifyy" on PyPI), version 0.9.61
  license: Apache License 2.0 (SPDX-License-Identifier: Apache-2.0; see the graphify
           project's LICENSE / LICENSE-MIT / NOTICE files)

Community detection on NetworkX graphs (Leiden via graspologic-native/graspologic, falling back
to Louvain). Used here to cluster the study-app concept graph into topics (SPEC.md §8). Vendored
as-is because its only dependency beyond the stdlib is networkx (graspologic-native is optional
and already a project dependency) - no code changes needed.
"""
